...
int main(){
...

    {
        ...
        std::unique_ptr<const AbstractNativeDerivProvider<double>> HD(new CoolPropNativeDerivProvider(HEOS, T, mapped_rhovec));
        std::unique_ptr<const MixDerivs<double> > (new MixDerivs<double>(std::move(HD)));
        auto toc = std::chrono::high_resolution_clock::now();
        std::cout << std::chrono::duration<double>(toc - tic).count()*1e6 << " us" << std::endl;
    }

    std::unique_ptr<const AbstractNativeDerivProvider<double> > HD(new CoolPropNativeDerivProvider(HEOS, T, mapped_rhovec));
    std::unique_ptr<const MixDerivs<double> > _cp(new MixDerivs<double> (std::move(HD)));
    const MixDerivs<double> &cp = *(_cp.get());
